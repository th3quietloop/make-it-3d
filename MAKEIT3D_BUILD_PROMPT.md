# Make It 3D: Build Prompt

You are building Make It 3D, a complete macOS app that converts 2D video into Apple spatial video (MV-HEVC) for Vision Pro. You are building it for Russell White on luna-pro (Apple silicon, macOS 15+). MAKEIT3D_PRD.md and MAKEIT3D_DESIGN.md sit next to this file. Read both fully before writing any code. The PRD defines what and why. The design file defines exactly how it looks and behaves; Phase 3 implements it verbatim.
...
3. Copy MAKEIT3D_PRD.md, MAKEIT3D_DESIGN.md, and this file into the repo root.
...
5. Build loop for the whole session: xcodegen generate, then xcodebuild -project Make It 3D.xcodeproj -scheme Make It 3D -configuration Debug -derivedDataPath ./build build, then open the built .app from ./build/Build/Products/Debug.
...
MakeIt3D/
  App/
    MakeIt3DApp.swift          main scene, Settings scene, menu commands
  ...
  Features/
    ...
    EmptyState/
  DesignSystem/
    Tokens.swift             every color, spacing, radius, type token from MAKEIT3D_DESIGN.md
    Components/
  Resources/
...
Gate: the app matches MAKEIT3D_DESIGN.md, state for state, token for token.
...
8. Golden set support: a debug menu item scans ~/Movies/MakeIt3DGoldenSet, queues everything found, and writes a dated verification report per export.
...
- README covers: what Make It 3D is, build loop commands, the AirDrop verification step, the sandbox decision, and where the golden set lives
