...
enum DepthEstimatorError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .modelMissing:
            return "The depth model is missing. Make It 3D can't read depth without it."
        case .modelLoadFailed(let detail):
            return "The depth model wouldn't load. \(detail)"
        case .unexpectedModelInterface(let detail):
            return "The depth model has an interface Make It 3D doesn't recognize. \(detail)"
...
/// Make It 3D reads that at runtime rather than hardcoding it, so swapping in a
