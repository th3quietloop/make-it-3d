...
///
/// Run `Make It 3D --makeicon` to regenerate the asset catalog.
enum IconRenderer {
    ...
    static func writeIconSetAndICNS(to directory: URL) throws {
        let iconSetURL = directory.appendingPathComponent("Make It 3D.iconset")
        ...
        process.arguments = [
            ...
            "-o", directory.appendingPathComponent("Make It 3D.icns").path
        ]
        try process.run()
    ...
    /// `Make It 3D --makeicon [catalog path]`
    static func runFromCommandLine() -> Bool {
        ...
        let catalogURL = URL(fileURLWithPath: catalogPath ?? FileManager.default.currentDirectoryPath
            + "/MakeIt3D/Resources/Assets.xcassets")
        ...
        // the asset catalog put a second Make It 3D.icns in the target and the
        ...
        do {
            ...
            print("Wrote Make It 3D.icns to \(artifactsURL.path)")
            return true
        } catch {
