...
enum SpatialWriterError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unsupportedEncoder:
            return "This Mac can't encode MV-HEVC, so Relief can't write a spatial video here."
        case .setupFailed(let detail):
            return "Couldn't start writing. \(detail)"
        case .writeFailed(let detail):
            return "Writing stopped. \(detail)"
        case .diskFull:
            return "The disk filled up. Relief removed the partial file. Free some space and try again."
        case .cancelled:
            return "Cancelled."
        }
    }
...
final class SpatialWriter: SpatialVideoWriting {
    ...
    /// is macOS 26 and later. Relief targets macOS 15, so the unrefined C entry
